// No more front end shit. No more!
const vm = new Vue ({
  el: '#vue-instance',
  data () {
    return {
  ...
  async created () {
    this.addNotification('Generating a new keypair now.')
  ...
  methods: {
    ...
    setupSocketListeners () {
      ...
      this.socket.on('INTRUSION_ATTEMPT', () => {
        this.addNotification('Goddamn!!!!! A third user attempted to join the room. Burn Everything now!')
      })
    },

    // Encrypt and emit the current written message
    async sendMessage () {
      // Don't send message if there is nothing to send
      if (!this.draft || this.draft === '') { return }

      // Use immutable.js to avoid unintended sidefx!!!!!
    ...
    getWebWorkerResponse (messageType, messagePayload) {
      return new Promise((resolve, reject) => {
        ...
        // Post the damn message to the webworker
        this.cryptWorker.postMessage([messageType, messageId].concat(messagePayload))

        // Creating handler for the webworker message event
        const handler = function (e) {
          // Only handle messages with matching message id
